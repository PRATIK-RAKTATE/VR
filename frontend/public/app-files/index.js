...
(function () {
  ...
  function switchScene(scene) {
    ...
    // Play audio for the scene if it has an audio property
    if (scene.data.audio) {
      audioController.play(scene.data.audio);
    }
  }
  ...
  function createLinkHotspotElement(hotspot) {
    ...
    wrapper.addEventListener("click", function () {
      // Play audio when hotspot is clicked
      var targetSceneData = findSceneDataById(hotspot.target);
      if (targetSceneData && targetSceneData.audio) {
        audioController.play(targetSceneData.audio);
      }
  ...
  function createInfoHotspotElement(hotspot) {
    ...
    var toggle = function () {
      // Play audio when info hotspot is clicked
      if (window.APP_DATA.scenes[0].audio) {
        audioController.play(window.APP_DATA.scenes[0].audio);
      }
  ...
  const sceneDropdown2 = document.getElementById("sceneDropdown2");

  sceneDropdown2.addEventListener("change", function () {
    const selectedSceneId = this.value;
    const selectedScene = findSceneById(selectedSceneId);
    if (selectedScene) {
      switchScene(selectedScene);
